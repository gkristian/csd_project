...
class SharedContext (object):
    def __init__(self):
        ...
        self.cpm_total_fwd_packets = 0
        #self.cpm_stats = {'conn_time': 0}
...
class CPM(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # cpm_connection_stat logger
        self.cpm_conn_stat_logger = logging.getLogger("cpm_conn_stat" + __name__)
        self.cpm_conn_stat_logger.setLevel(logging.DEBUG)
        handler = logging.FileHandler("/var/www/html/spacey/cpm_conn_stat.log")
        handler.setLevel(logging.DEBUG)
        # formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
        handler.setFormatter(formatter)
        self.cpm_conn_stat_logger.addHandler(handler)
        self.cpm_conn_stat_logger.info("#total_num_of_packets_forwarded, time_taken_to_fwd")
    ...
    def __check_bootstrap_completion(self):
        ...
        #Delete below block
        bcomplete = not self.defines_D['bootstrap_in_progress']
        #if self.bootstrap_complete and self.defines_D['temp_bstrap_print_once']:
        if bcomplete and self.defines_D['temp_bstrap_print_once']:
            ...
            if (self.config.save_topology_to_file_post_bootstrap_only_once):
                ...
                self.cpm_bstrap_logger.debug("In checkbootstrap completion, Saved topology to file")
            self.print_l2_table()
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst_mac == self.defines_D['bcast_mac']: #if dst == 'ff:ff:ff:ff:ff:ff' means its a host that is flooding the network, we gotta learn it
            ...
            if dst_mac in self.net:
                ...
                # We can route this packet as we do have a path to it to start measuring fwd time
                #fwd_time_start = int(time.time())
                cpm_stat_conn_time = 0.0  # CPM connetion time , a statistic required by teacher for CPM to report. see the teacher's given document
                #fwd_time_start = int(time.time() * 1000000)  # time in ms at which a packet came to us to be forwarded
                #fwd.time_start = os.times()[4]
                fwd_time_end = 0.0  # time at which we finished off all requirements like installing switch paths to forward the packet
                #from datetime import dateime as dt
                #fwd_time_start = dt.now()
                """
                Tip: Don't use python for benchmarking. Not time.time() , not datetime.now(),only
                last resort is timeit module according to the web
                confirmed by teacher as well that Cython with C time systme calls are the proper way to benchmark
                """
                #below causes the shorest_path call to be made and time it take to compute the cpm_connection stat
                wrapped = self.wrapper(self.__shortest_path_computer_and_install,src_mac,dst_mac)
                cpm_stat_conn_time=timeit.timeit(wrapped,number=1)
                cpm_stat_conn = 1000000
                #self.__shortest_path_computer_and_install(src_mac, dst_mac)

                #fwd_time_end = dt.now()
                #cpm_stat_conn_time = (fwd_time_end-fwd_time_start).microseconds
                #cpm_stat_conn_time = timeit.timeit(self.__shortest_path_computer_and_install(src_mac,dst_mac))
                #fwd_time_end = self.epoc_starttime = int(time.time())
                #fwd_time_end = int(time.time() * 1000000)  #time in milliseconds
                #fwd_time_end = os.times()[4]

                #cpm_stat_conn_time = fwd_time_end - fwd_time_start

                self.shared_context.cpm_total_fwd_packets = self.shared_context.cpm_total_fwd_packets + 1
                self.cpm_conn_stat_logger.debug(";%r,%r", self.shared_context.cpm_total_fwd_packets, cpm_stat_conn_time)

                # fetch metrics from a remote source and add to graph and compute weight for each link
                #move to get topology_data as  below will only trigger when we receive a packet
    ...
    def get_topology_data(self, ev):
        if self.shared_context.bootstrap_complete:
            self.__log_all_graph("Before WEIGHT_ADDED_TO_TOPOLOGY")
            self.__fetch_ALL_metrics_and_insert_weight_in_topology_graph()
            self.__log_all_graph("After WEIGHT_ADDED_TO_TOPOLOGY")
    ...
    def __fetch_ALL_metrics_and_insert_weight_in_topology_graph(self):
        ...
        # Is it time to fetch the metrics from DM , observe timer
    ...
    def wrapper(self,func, *args, **kwargs):
        """
        Used by timeit benchmarking library which requires function to benchmarked to be wrapped
        :param args:
        :param kwargs:
        :return:
        """
