...
# Ryu controller won't run on a linux without x-windows unless an Agg
# backend is selected
import os
...
class CPM(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # fetch graph object of physical network
        ...
        self.logger.debug(
            "SHARED_CONTEXT : SharedContext Instantiated and time_of_last_fetech = %r",
        ...
        # Module set to True will have their metric data fetched using
        # REST(GET) by the CPM
        ...
        # module and helps segregates logs of one module fron the other during
        # troubleshooting. CPM can still log to the ryu logger under normal
        # operation if needed.
        ...
        self.edge_weights = dict.fromkeys(
            ['core_util', 'mem_usage', 'rpm_weight', 'link_util', 'sw_packet_drop_rate'])
        ...
        # parameter described below, TODO: do as enumeration
        self.network_bootstrap_type = 1
        """
        ...
        # used by type 1, means stop after 4 mac addresses learnt
        self.network_bootstrap_discovery_count = 4

        self.datapathDb = {}
        self.already_installed_paths_SET = set()
        self.cpmlogger.info(
            "BOOTSTRAP, type = %r , host_count = %r",
            self.network_bootstrap_type,
            self.network_bootstrap_discovery_count)
    ...
    """
    # An openflow v1.3 implementation must install a table-miss flow entry
    # rule while version 1.0 doesn't require this.
    ...
    def __check_bootstrap_completion(self):
        ...
        if self.defines_D['bootstrap_in_progress']:
            if (self.network_bootstrap_type == 0):
                if int(time.time()) - \
                        self.epoc_starttime > self.network_bootstrap_time:
                    self.defines_D['bootstrap_in_progress'] = False
                    self.cpm_bstrap_logger.debug(
                        "Bootstrap type %d just Completed",
                        self.network_bootstrap_type)
            if (self.network_bootstrap_type == 1):
                if len(
                        self.l2_mac2ip_table) == self.network_bootstrap_discovery_count:
                    self.defines_D['bootstrap_in_progress'] = False
                    self.cpm_bstrap_logger.debug(
                        "Bootstrap type %d just Completed",
                        self.network_bootstrap_type)
        # Update the below variable that is shared through _CONTEXT_ we keep
        # this variable seperate for now
        self.bootstrap_complete = not self.defines_D[
            'bootstrap_in_progress']  # xxxxxxxxxxxxxxxxx
        self.shared_context.bootstrap_complete = not self.defines_D[
            'bootstrap_in_progress']
        # Delete below block

        if self.bootstrap_complete and self.defines_D[
                'temp_bstrap_print_once']:
            self.cpm_bstrap_logger.info(
                "CPM : *******    BOOTSTRAP COMPLETE   ********")
            self.print_l2_table()
    ...
    def __remove_macs_from_shortest_path(self, spath):
        ...
        :return:
        """
        # spath_dq = collections.deque(spath) #this was giving ERROR #CSD CC 2
        # which makes sense
        spath_dq = deque(spath)
        dst_mac = spath_dq.pop()  # removes last element from the list
        # removes first element from the list, not that classical python list
        # manipulations are slow as compared to this
        src_mac = spath_dq.popleft()
    ...
    def __install_path_flow(self, spath_with_macs):
        ...
        # nr of switches in the path + 2 i.e.including src mac and dst mac
        n = len(spath_with_macs)
        i = 1
        # during first iterations its the src_mac
        src_mac = spath_with_macs[i - 1]
        ...
        while (i <= n - 2):  # n-1 is the last node which is the dst mac
            ...
            # this switch already has flow installed during first iteration
            sw_b = spath_with_macs[i]
            sw_c = spath_with_macs[i + 1]  # on sw_b , flow is to be installed
            sw_a = spath_with_macs[i - 1]  #

            in_port = self.net.edge[sw_a][sw_b]['dst_port']
            self.cpm_route_logger.debug(
                "FWD FLOW in_port computed = self.net.edge[%r][%r]['dst_port'] = %r",
                sw_a,
                sw_b,
                in_port)
            out_port = self.net.edge[sw_b][sw_c]['src_port']
            self.cpm_route_logger.debug(
                "FWD FLOW out_port computed = self.net.edge[%r][%r]['src_port'] = %r ",
                sw_b,
                sw_c,
                out_port)
        ...
        while (i >= 1):  # 1 is the first switch
            ...
            in_port = self.net.edge[sw_a][sw_b]['dst_port']
            self.cpm_route_logger.debug(
                "REV FLOW in_port computed = self.net.edge[%r][%r]['dst_port/in_port'] = %r",
                sw_a,
                sw_b,
                in_port)
            out_port = self.net.edge[sw_b][sw_c]['src_port']
            self.cpm_route_logger.debug(
                "REV LOW out_port computed = self.net.edge[%r][%r]['src_port/out_port'] = %r",
                sw_b,
                sw_c,
                out_port)
            self.__send_flow_mod(sw_b, src_mac, in_port, dst_mac, out_port)
    ...
    # self.__send_flow_mod(self.datapathDb(dpid), src_mac,in_port, dst_mac,
    # out_port) #, in_port, out_port)
    ...
    def __send_flow_mod(self, dpid, src_mac, in_port, dst_mac, out_port):
        self.cpm_route_logger.debug(
            "FLOWMOD_1 : installing rule: %r -----> %r ==== %r === %r -----> %r ",
            src_mac,
            in_port,
            dpid,
            out_port,
        ...
        match = ofp_parser.OFPMatch(
            in_port=in_port,
            eth_dst=dst_mac,
            eth_src=src_mac)  # 'ff:ff:ff:ff:ff:ff'
        # match = ofp_parser.OFPMatch(
        #     in_port=1,
        #     eth_dst=dst_mac,
        #     eth_src=src_mac,
    ...
    def __isPathNotAlreadyInstalled(self, spath):
        ...
        # set can only contain a hashable immutable object, a list is not
        # acceptable.
        if spath_as_tuple in self.already_installed_paths_SET:
            # this computed shortest path is already installed in the switches
            self.logger.debug(
                "___PATH_ALREADY_INSTALLED____ i.e. %r",
                spath_as_tuple)
            return True
        else:

            # A list cannot be added to a set because it is not hashable, while
            # a tuple can be added cuz its not mutable but hashable.
            self.already_installed_paths_SET.add(spath_as_tuple)
            self.logger.debug(
                "---PATH_NOT_ALREADY_INSTALLED---- i.e. %r",
                spath_as_tuple)
            return False
    ...
    def _packet_in_handler(self, ev):
        ...
        if self.defines_D['enable_save_topology_to_file']:
            # this should go away in the production version and above lines be
            # uncommented in a proper manner
            self.save_topolog_to_file()
        msg = ev.msg
        datapath = msg.datapath
        # we save all datapaths in a dictionary so that the controller can
        # install flowmod rules in any switch it wants
        ...
        # #this on pingall gave: '\xff\xff\xff\xff\xff\xff'

        if dst_mac == self.defines_D[
                'bcast_mac']:  # if dst == 'ff:ff:ff:ff:ff:ff' means its a host that is flooding the network, we gotta learn it
            self.cpm_bstrap_logger.debug(
                "broadcast received from src mac = %s , switch dpid = %r at in_port = %r",
                src_mac,
                dpid,
                msg.match['in_port'])
            ...
            # this object does not have any indexes so no [0]
            ...
            if src_mac not in self.net:  # learn it
                self.cpm_bstrap_logger.debug(
                    "LEARNING : learning a new src_mac = %r for the first time ", src_mac)
                #self.logger.debug("our current l2_table is %r", self.l2_dpid_table)
                # open for IP src and IP dst of ARP broadcast: we  associate
                # src mac learn from source and use destination IP to route to
                # correct switch
                ...
                # below three tables could be improved by putting all info in
                # one object and use getters and setters but I dont know if it
                # would be as efficent as having below dictionary as a lookup
                # table
                self.l2_dpid_table[dpid] = {
                    src_mac: {
                        'in_port': msg.match['in_port'],
                        'ip': pkt_arp.src_ip}}
                ...
                self.cpm_bstrap_logger.debug(
                    "LEARNING : ARP extractions show about packet details as : arp.src_ip = %r , arp.dst_ip = %r , arp.opcode (1 for Request)  = %r  ",
                    pkt_arp.src_ip,
                    pkt_arp.dst_ip,
                    pkt_arp.opcode)
                # since its a directonal graph so we need to add two pairs for
                # every single links i.e in each direction

                self.net.add_edge(src_mac,
                                  dpid,
                                  {'src_port': msg.match['in_port'],
                                   'dst_port': msg.match['in_port'],
                                      ...
                                      'ip': pkt_arp.src_ip,
                                      'bw': 10,
                                      'weight': 0})  # src is the src mac

                self.net.add_edge(dpid,
                                  src_mac,
                                  {'src_port': msg.match['in_port'],
                                   'dst_port': msg.match['in_port'],
                                      'src_dpid': dpid,
                ...
                if pkt_arp.opcode != arp.ARP_REQUEST:
                    self.cpm_bstrap_logger.debug(
                        "IGNORING packet as its not an arp request")
                    return
                self.cpm_bstrap_logger.debug(
                ...
                # have we learnt the IP in arp.dst_ip header ie. do we know its
                # mac yet
                if pkt_arp.dst_ip not in self.l2_ip2mac_table:
                    self.cpm_bstrap_logger.debug(
                        "RX_BCAST..:Src Mac known but havent learnt the target IP yet i.e. arp.dst_ip's mac yet. Return")
                    return
                ...
                self.cpm_bstrap_logger.debug(
                    "KEY1 : Found dst_mac %r for arp header dst ip %r , puttin in arp reply",
                    mac_of_arp_dst_ip,
                    pkt_arp.dst_ip)
                ...
                self.cpm_bstrap_logger.debug(
                    "Crafting an ARP reply")  # pkt_arp.src_mac

                # self._send_packet(datapath,
                # self.l2_dpid_table[dpid][src_mac]['in_port'], new_pkt) #dpid=
                # datapath.id the switch the packet in came from we want to
                # reply on that switch
                # dpid= datapath.id the switch the packet in came from we want
                # to reply on that switch
            ...
            # This is if block ensures that code after it only gets executed
            # once the network has bootstraped i.e. bootstrap time limit has
            # reached
            if self.defines_D['bootstrap_in_progress']:
                self.cpm_bstrap_logger.info(
                    "RX_NO_BCddAST_ONLY_TARGETED_DST_MAC : Sorry network bootstrap still in progress not computing spath , not installing any flows")
            ...
            self.cpm_route_logger.debug(
                "__________RX_NO_BCAST_ONLY_TARGETED_DST_MAC : Network Bootstrap Completed. Proceeding with shortest path calculation________")
            ...
            if dst_mac in self.net:
                if self.disable_cpm_openflow_ruleinstaller:
                    ...
                    """
                    self.cpm_route_logger.warning(
                        "cpm_openflow_ruleinstaller is disabled in config, Not installing any openflow rules in the switches")
                    return
                self.cpm_route_logger.debug(
                    "SPATH: Received ARP to specific dst mac %r that exist in our graph", dst_mac)
                self.cpm_route_logger.debug(
                    "SPATH Do we have a path to this destination mac? src= %r , dst = %r ",
                    dst_mac,
                    src_mac)
                if not nx.has_path(
                        self.net, src_mac, dst_mac):  # if returned False we abort
                    # Above line once caused error CSD_CC_1 reported in
                    # ERRORS.txt
                    self.cpm_route_logger.debug(
                        "Cannot find path from src mac %r to dst_mac %r, returning ie. doing nothing more",
                        src_mac,
                        dst_mac)
                    return
                # fetch metrics from a remote source and add to graph and
                # compute weight for each link
                ...
                # None of below exceptions should trigger since I already
                # checked if a shortest path exists or not earlier. But I am
                # going to put exceptions just to be on safe side.
                if self.config.enable_weighted_routing:
                    if self.config.weighted_routing_type == 'dijkrsta':
                        self.cpm_route_logger.debug(
                            "WEIGHTED_ROUTING enabled, type = %r",
                            self.config.weighted_routing_type)
                        try:
                            spath = nx.dijkstra_path(
                                self.net, src_mac, dst_mac)
                        except Exception:
                            self.cpm_route_logger.debug(
                                "No weighted shortest path exist between src = %r and dst = %r", src_mac, dst_mac)
                else:
                    self.cpmlogger.debug("WEIGHTED_ROUTING disabled")
                    try:
                        # If there are more than one shortest path between the
                        # source and target, it would just return one of them
                        spath = nx.shortest_path(self.net, src_mac, dst_mac)
                    except Exception:
                        self.cpm_route_logger.debug(
                            "No shortest path exist between src = %r and dst = %r", src_mac, dst_mac)

                self.logger.debug(
                    "Found shortest path from src mac %r to dst mac %r as %r",
                    src_mac,
                    dst_mac,
                    spath)
                ...
                self.cpm_route_logger.debug(
                    " Unable to find path from  src_mac=%r to dst_mac=%r  ", dst_mac, src_mac)
                self.cpm_bstrap_logger.debug(
                    "Shall we Learn new MAC dst_mac=%r src_mac=%r ", dst_mac, src_mac)
    ...
    def save_topolog_to_file(self):
        ...
        filename_src = self.defines_D['logdir'] + \
            '/CPM_network_with_src_port.png'
        ...
        filename_dst = self.defines_D['logdir'] + \
            '/CPM_network_with_dst_port.png'
        ...
        # plt.show() #Do not uncomment this in a realtime application. This
        # will invoke a standalone application to view the graph.

        plt.savefig(filename_dst)
        # this cleans the palette for the next time, otherwise it will keep on
        # drawing the same image.
        plt.clf()
    ...
    def show_graph_stats(self):
        if self.defines_D['bootstrap_in_progress']:
            ...
            self.cpm_bstrap_logger.debug(
                "list of edges: self.net.edges %s ", self.net.edges())
            self.cpm_bstrap_logger.debug(
                "list of nodes: \n %r", self.net.nodes())
            self.cpm_bstrap_logger.debug(
                "l2_lookup_table : %r", self.l2_dpid_table)
    ...
    # Below method just populates the net graph to which above packet-in
    # method just adds src/dst
    ...
    def get_topology_data(self, ev):
        ...
        # self.logger.debug("self.ls(links_list): %r", self.ls(links_list))
        ...
        # links_dict = [self.logger.debug("link.to_dict() = %r",
        # link.to_dict()) for link in links_list] #this in log shows as below:
        ...
        # links_type = [self.logger.debug("type(link) = %r", type(link)) for
        # link in links_list] #this shows <ryu.topology.switches.Link
        ...
        # Since "net" is a Directional Graph so we need to add two ordered
        # pairs srcdpid-srcinport,dstdpid-dstinport,{'weight',100}) for a link
        # between switch srcdpid and dstdpid
        ...
        # Storing the src and dst dpid key in the dictionary is redundant but I
        # want to experiment with something later.
        ...
        """
        # while installing flow rules observed that src and dst port are
        # opposite to our understanding (BUG: in_port and out_port are the same
        # as infact this)

        links_onedirection_L = [(link.src.dpid,
                                 link.dst.dpid,
                                 {'dst_port': link.src.port_no,
                                  'src_port': link.dst.port_no,
                                  'dst_name': link.src.name,
                                  'src_name': link.dst.name,
                                  'bw': 10,
                                  'weight': 0}) for link in links_list]
        links_opp_direction_L = [(link.dst.dpid,
                                  link.src.dpid,
                                  {'src_port': link.dst.port_no,
                                   'dst_port': link.src.port_no,
                                   'dst_name': link.src.name,
        ...
        # since its a directional graph it must contain edges in opposite
        # direction as well.
        self.net.add_edges_from(links_opp_direction_L)
        # self.show_graph_stats()
    ...
    def __log_all_graph(self, msg):
        ...
        """
        self.cpmlogger.info(
            "________________ ALL_TOPOLOGY_PRINT begin  : %r________", msg)
        for src, dst, edge_data in self.net.edges_iter(data=True):
            self.cpmlogger.debug(
                "WEIGHTED_TOPOLOGY FINAL: (%r,%r, %r)",
                src,
                dst,
                edge_data)  # even self.net() might do as well
        self.logger.info(
            "________________ ALL_TOPOLOGY_PRINT_TOPOLOGY end________")
        self.show_graph_stats()

    def print_l2_table(self):
        self.cpm_bstrap_logger.debug(
            "CPM BSTRAP_TABLES : l2_table = %r",
            self.l2_dpid_table)
        self.cpm_bstrap_logger.debug(
            "CPM BSTRAP_TABLES : l2_mac2ip = %r",
            self.l2_mac2ip_table)
        self.cpm_bstrap_logger.debug(
            "CPM BSTRAP_TABLES : l2_ip2mac = %r",
            self.l2_ip2mac_table)
    ...
    def __update_graph(self, src_dpid, dst_dpid, key, value):
        ...
        self.cpmlogger.info(
            "UPDATE_GRAPH , src_dpid = %r, dst_dpid = %r, weight = %r",
            src_dpid,
            dst_dpid,
            value)
        ...
        try:
            self.cpmlogger.debug(
                "UPDATE_GRAPH src_dpid type  =%r and dst_dpid type = %r and key = %r and type of key = %r ",
                type(src_dpid),
                type(dst_dpid),
                key,
                type(key))
            ...
            self.cpmlogger.debug(
                "UPDATE_GRAPH Assigned value self.net.edge[%r][%r][%r] = %r",
                src_dpid,
                dst_dpid,
                key,
            ...
            self.cpmlogger.error(
                ...
                self.net.edge[src_dpid][dst_dpid]['weight'])
            self.cpmlogger.error(
                "UPDATE_GRAPH Handled exception KeyError when updating graph",
                exc_info=True)
            # raise
        except (NameError):
            self.cpmlogger.error(
                "UPDATE_GRAPH Handled exception NameError when updating graph",
                exc_info=True)
        except Exception as e:
            #self.logger.exception("Unable to update this key in the graph on fetch %r",e)
            self.cpmlogger.error(
                'Unable to update this key in the graph, exception caught and dealt, here is the traceback',
                exc_info=True)
    ...
    def __fetch_ALL_metrics_and_insert_weight_in_topology_graph(self):
        ...
        self.logger.debug(
            "FETCH_ALL_METRICS : fetch_metric_and_insert, time_max_limit = fetch every %r seconds",
            time_max_limit)
        if not (self.current_time - self.shared_context.time_of_last_fetch >
                time_max_limit) and self.defines_D['bootstrap_in_progress']:
            return
        else:
            self.shared_context.time_of_last_fetch = self.current_time

        self.cpmlogger.debug(
            "FETCH_ALL_METRICS : About to fetch remote metrics, time_of_last_fetch = %r",
            self.shared_context.time_of_last_fetch)
        # Fetch metrics data
        ...
        for src_node, dst_node, data in self.net.edges_iter(data=True):
            ...
            try:
                ...
                self.logger.error(
                    "Program is running normally but exception encountered when parsing a graph node =%r for : ",
                    e)
                # exc_info causes a Trace exception details to be printed to
                # log but it does not halt program execution
                self.logger.error(
                    "Program runs normally but Exception encountered when parsing a graph node =%r for : ",
                    src_to_dst_node,
                    exc_info=True)
            ...
            link_weight = self.__compute_link_weight(unicode(src_node), unicode(
                dst_node), nfm_metrics_data, rpm_metrics_data, hum_metrics_data)
            self.cpmlogger.debug(
                "FETCH_ALL: about to call update_graph with src_node = %r, dst_node = %r",
                src_node,
                dst_node)
    ...
    def __compute_link_weight(self, src_node, dst_node, nfm, rpm, hum):
        ...
        hum_total_weight = 0
        # nfm would be False if HTTP GET of nfm_metric_data returned blank or
        ...
        if self.modules_enabled['NFM'] and nfm:
            ...
            if nfm_dict_key in nfm_packet_dropped:
                nfm_packet_dropped_link_value = float(
                    nfm_packet_dropped[nfm_dict_key])
            else:
                nfm_packet_dropped_link_value = 0

            nfm_total_link_weight = nfm_link_util_weight * nfm_link_util_value + \
                nfm_packet_dropped_weight * nfm_packet_dropped_link_value
            # All modules contribute equally to the output weight, see Metric
            # desc. document and weights specified by the teacher
            nfm_total_link_weight = 0.33 * nfm_total_link_weight
        self.cpmlogger.debug(
            "CALC_NFM_WEIGHT, src_node = %r , dst_node = %r, nfm_total_link_weight =  %r",
            src_node,
            dst_node,
            nfm_total_link_weight)

        # RPM would be False if HTTP GET of rpm_metric_data returned blank or
        # failed due to connection error
        if self.modules_enabled['RPM'] and rpm:
            rpm_total_weight = 0
            rpm_src_node_weight = 0
            rpm_dst_node_weight = 0
            rpm_src_node_weight = self.__compute_rpm__weight_value_for_node(
                src_node, rpm)
            rpm_dst_node_weight = self.__compute_rpm__weight_value_for_node(
                dst_node, rpm)
            rpm_link_weight = rpm_src_node_weight + rpm_dst_node_weight

            # All modules contribute equally to the output weight
            rpm_total_link_weight = 0.33 * rpm_link_weight
            self.cpmlogger.debug(
                "CALC_RPM_WEIGHT, src_node = %r , dst_node = %r, total_rpm_weight =  %r",
                src_node,
                dst_node,
                rpm_total_link_weight)
        ...
        if self.modules_enabled['HUM'] and hum:
            ...
            # CPU utilization is more critical in our testbed where memory is
            # not limited by the size of a TCAM
            hum_core_weight = 0.9
            ...
            hum_total_weight = 0
            try:
                hum_total_weight = hum_core_weight * \
                    sum(hum_core_util_dict.itervalues()) + hum_memory_weight * float(hum_memory)
            except Exception:
                self.cpmlogger.debug(
                    "Hum metrics data empty or invalid - seting hum_total_weight to 0")
                hum_total_weight = 0  # better safe than sorry

            hum_total_weight = 0.333 * hum_total_weight
            self.cpmlogger.debug(
                "hum_total_weight contributed to link weight is = %r",
                hum_total_weight)

        link_weight = nfm_total_link_weight + rpm_total_link_weight + hum_total_weight
        return link_weight

    def __compute_rpm__weight_value_for_node(
            self, node_dpid, rpm_metrics_data):
        """
        ...
        # Below are values specified by the teacher as a requirement (see
        ...
        try:
            node_25_value = latency_topo_dict[
                switch_dpid_in_unicode][key_latency_25]
        except Exception:
            node_25_value = 0
            self.cpmlogger.debug(
                "CALC_RPM_WEIGHT MISS: 25 value exception hit but its expected as RPM metric data received may not contain value for that node. topology subset")
            self.cpmlogger.debug(
                "CALC_RPM_WEIGHT MISS: RPM HTTP get does not contain info for node_dpid = %r",
                node_dpid)

        try:
            node_75_value = latency_topo_dict[
                switch_dpid_in_unicode][key_latency_75]
        except Exception:
            node_75_value = 0
            self.cpmlogger.debug(
                "CALC_RPM_WEIGHT MISS: 75 value exception hit but its expected as RPM metric data received may not contain value for that node. topology subset")
            self.cpmlogger.debug(
                "CALC_RPM_WEIGHT MISS: RPM HTTP get does not contain info for node_dpid = %r",
                node_dpid)

        try:
            node_median_value = latency_topo_dict[
                switch_dpid_in_unicode][key_latency_median]
        except Exception:
            node_median_value = 0
            self.cpmlogger.debug(
                "CALC_RPM_WEIGHT MISS: median value exception hit but its expected as RPM metric data received may not contain value for that node. topology subset")
            self.cpmlogger.debug(
                "CALC_RPM_WEIGHT MISS: RPM HTTP get does not contain info for node_dpid = %r",
                node_dpid)

        self.logger.debug(
            " CALC_RPM_WEIGHT : computed 25,75,median values are: %r , %r , %r ",
            node_25_value,
            node_75_value,
            node_median_value)
        if node_median_value < 500 and node_median_value > 5000:
            self.cpmlogger.warning(
                "CALC_RPM_WEIGHT : UN_NORMALIZED_MEDIAN_VALUE received from Cache. As per spec. doc it must be between 500 and 5000")
        ...
        total_rpm_node_value = node_25_value * WEIGHTAGE_LATENCY_25 + node_75_value * \
            WEIGHTAGE_LATENCY_75 + node_median_value * WEIGHTAGE_LATENCY_MEDIAN
        return total_rpm_node_value

    def __REST_get_NFM_metrics(self):
        nfm_metrics_data = True
        nfm_what_metrics_to_fetch = {
            'module': 'nfm', 'keylist': [
                'link_utilization', 'packet_dropped']}
        try:
            nfm_metrics_data = self.DMclient.getme(nfm_what_metrics_to_fetch)
        except Exception as e:
            self.cpmlogger.error(
                "FETCH_NFM_METRICS : HTTP Failure ...., Exception = %r", e)
            self.cpmlogger.error(
                "FETCH_NFM_METRICS : HTTP Failure ...., Exception trace",
                exc_info=True)
            nfm_metrics_data = False
            return nfm_metrics_data
        else:
            # See my controller_core/tests/rest_nfm_get_with_packet_drops.py
            # test script for more details
            self.cpmlogger.debug(
                "FETCH_NFM_METRICS: -----> nfm_metrics_data  = %r ",
                nfm_metrics_data)
            # empty dicitionary like string evaluates to False
            if not(nfm_metrics_data[0][1] and nfm_metrics_data[1][1]):
                self.cpmlogger.error(
                    "FETCH_NFM_METRICS : Empty NFM data read, key value, either or both of link_util or packet_drop is empty. DM,DB running but blank data served.")
                self.cpmlogger.error(
                    "FETCH_NFM_METRICS : nfm_metrics_data = %r",
                    nfm_metrics_data)
                nfm_metrics_data = False
        ...
        self.cpmlogger.debug(
            "FETCH_NFM_METRICS: HTTP GET RESULT - nfm_metrics_data  = %r ",
            nfm_metrics_data)
    ...
    def __REST_get_HUM_metrics(self):
        ...
        try:
            ...
            self.cpmlogger.error(
                "FETCH_HUM_METRICS : HTTP Failure ...., Exception = %r", e)
            self.cpmlogger.error(
                "FETCH_HUM_METRICS : HTTP Failure ...., Exception trace",
                exc_info=True)
            ...
            if not(hum_metrics_data[0][1] and hum_metrics_data[1][1]):
                self.cpmlogger.error(
                    "FETCH_HUM_METRICS : Empty HUM data read, key value, either or both of link_util or packet_drop is empty. DM,DB running but blank data served")
                hum_metrics_data = False

        self.cpmlogger.debug(
            "FETCH_HUM_METRICS: OK - hum_metrics_data  = %r ",
            hum_metrics_data)
    ...
    def __REST_get_RPM_metrics(self):
        ...
        try:
            ...
            self.cpmlogger.error(
                "FETCH_RPM_METRICS : HTTP Failure ...., Exception = %r", e)
            self.cpmlogger.error(
                "FETCH_RPM_METRICS : HTTP Failure ...., Exception trace",
                exc_info=True)
            ...
            # test script for more details
            self.cpmlogger.debug(
                "FETCH_RPM_METRICS: -----> EMPTY - rpm_metrics_data  = %r ",
                rpm_metrics_data)
            # empty dicitionary like string evaluates to False , [0][1] is the
            # dictionary of switch latencies and if its empty then its a
            # problem
            if not (rpm_metrics_data[0][1]):
                ...
                self.cpmlogger.error(
                    "FETCH_RPM_METRICS : nfm_metrics_data = %r",
                    rpm_metrics_data)
                rpm_metrics_data = False

        self.cpmlogger.debug(
            "FETCH_RPM_METRICS: HTTP GET RESULT - rpm_metrics_data  = %r ",
            rpm_metrics_data)
