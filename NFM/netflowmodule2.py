	...
	def __init__(self, *args, **kwargs):
		...
		while(True):
			...
			if self.shared_context.bootstrap_complete == True:
				...
				print("NFM NET NOT READY!")
			time.sleep(1)
		"""
		"""
		self.notRecivedGraph = True
		while self.notRecivedGraph:
			try:
				self.notRecivedGraph = False
		...
		#print(list(self.net.edge()))


	@set_ev_cls(ofp_event.EventOFPPacketIn, MAIN_DISPATCHER)
	def get_completed_net(self, ev):
		if self.shared_context.bootstrap_complete == True:
			print("NFM NET READY!")
			return
		else:
